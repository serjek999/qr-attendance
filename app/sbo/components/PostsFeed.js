...
const PostsFeed = ({ user }) => {
    ...
    const handleLike = async (postId) => {
        try {
            // First, try to check if the sbo_officer_id column exists by attempting a simple query
            const { data: testQuery, error: testError } = await supabase
                .from('post_likes')
                .select('sbo_officer_id')
                .limit(1);

            if (testError && testError.message && testError.message.includes('column "sbo_officer_id" does not exist')) {
                // The sbo_officer_id column doesn't exist, so we'll use a fallback approach
                console.log('sbo_officer_id column not found, using fallback approach');
            ...
            if (error.message) {
                if (error.message.includes('column "sbo_officer_id" does not exist')) {
