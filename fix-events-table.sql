...
-- This script will drop and recreate the events table with the correct structure

-- Drop existing tables if they exist
DROP TABLE IF EXISTS event_participants CASCADE;
DROP TABLE IF EXISTS events CASCADE;
DROP TABLE IF EXISTS tribe_scores CASCADE;
...
CREATE INDEX idx_events_tribe_id ON events(tribe_id);
CREATE INDEX idx_events_status ON events(status);
CREATE INDEX idx_tribe_scores_tribe_id ON tribe_scores(tribe_id);
CREATE INDEX idx_tribe_scores_category ON tribe_scores(category);
...
-- Insert some sample data for testing (only if tribes exist)
...
('Science Fair', 'Student science project exhibition', '2024-12-20', '14:00:00', 'School Auditorium', 'general', 'upcoming', 'admin'),
('Cultural Festival', 'Traditional dance and music performances', '2024-12-25', '18:00:00', 'School Hall', 'general', 'upcoming', 'admin')
ON CONFLICT DO NOTHING;

-- Insert sample tribe scores (only if tribes exist)
